    ...
    void generatedTest() {
        ...
        step("Ввести данные для регистрации", () -> {
            ...
            step("Ввести отчество", () -> {
                $("[name='REGISTER[SECOND_NAME]']").setValue(secondName);
            });
            step("Ввести e-mail", () -> {
                $("[name='REGISTER[EMAIL]']").setValue(testEmail);
            });
            step("Ввести номер телефона", () -> {
                $("[name='REGISTER[PERSONAL_PHONE]']").setValue(testPhone);
            });
            step("Ввести логин", () -> {
                $("[name='REGISTER[LOGIN]']").setValue(testLogin);
            });
            step("Ввести пароль", () -> {
                $("[name='REGISTER[PASSWORD]']").setValue(testPassword);
            });
            step("Ввести подтверждение пароля", () -> {
                $("[name='REGISTER[CONFIRM_PASSWORD]']").setValue(testPassword);
            });
        });

        step("Выбрать опции", () -> {
            step("Подтвердить доступ к демо-версиям", () -> {
                $(withText("Хочу получить доступ к демо-версиям программных продуктов:")).click();
            });
//            step("Подтвердить доступ к новостным рассылкам", () -> {
//                $(withText("Подписаться на новости Инсат")).click();
//            });
            step("Согласие на обработку персональных данных", () -> {
                $("[name='agreement']").sibling(0).click();
            });
        });
